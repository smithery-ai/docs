...
app.post('/oauth/token', async (req, res) => {
  ...
  if (grant_type === 'urn:ietf:params:oauth:grant-type:token-exchange') { // [!code highlight]
    // Verify the incoming Smithery OIDC ID token
    const smitheryToken = await verifySmitheryIdToken(subject_token); // [!code highlight]
    ...
    const user = await findOrCreateUser({ // [!code highlight]
      externalId: `smithery:${smitheryToken.sub}`, // [!code highlight]
      email: smitheryToken.email // [!code highlight]
    }); // [!code highlight]
...
async function verifySmitheryIdToken(token: string) {
  try {
    ...
    const { payload } = await jwtVerify(token, SMITHERY_JWKS, { // [!code highlight]
      issuer: 'https://server.smithery.ai', // [!code highlight]
      audience: '@your-org/your-server', // Your server's qualified name // [!code highlight]
      algorithms: ['RS256'] // [!code highlight]
    }); // [!code highlight]
    
    return payload;
    // Standard OIDC claims:
    // - iss: "https://server.smithery.ai"
    // - sub: user identifier
...
// auth.example.com/.well-known/oauth-authorization-server
{
  ...
  "grant_types_supported": [
    "authorization_code",
    "urn:ietf:params:oauth:grant-type:token-exchange" // [!code highlight]
  ],
  ...
}
```
...
This approach is compliant with the MCP OAuth specification and OAuth 2.0 standards.
...
- **Issuer**: Must be `https://server.smithery.ai`
