...
SHTTP (Server-Sent Events over HTTP) is ideal for web-based applications and services that need HTTP connectivity:
...
#### STDIO Transport
STDIO transport is perfect for command-line tools and desktop applications that communicate via standard input/output:
...
By default, the CLI reads your entry point from the `"module"` field in `package.json`:
...
# Or provide config as JSON
echo '{"method": "initialize", "params": {}}' | node .smithery/index.cjs