...
- Server that listens on a configurable `PORT` environment variable (Smithery will set this to 8081)
- **(Optional)** Server that can optionally handle session configuration passed by Smithery (API keys, settings, etc.) - see our [cookbook](/cookbooks/python_custom_container) for a complete example
...
5. Send MCP `initialize` and `list_tools` messages with a dummy configuration (via base64-encoded config parameter in URL) to discover your server's capabilities
...
- **Configuration errors**: Check that your server handles the base64-encoded config parameter in the URL
...
- **Parse the config parameter**: Extract and decode the base64-encoded config from the URL query parameter
