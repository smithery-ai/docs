...
Choose custom containers when you:
...
- Server that implements [Streamable HTTP](https://modelcontextprotocol.io/specification/2025-03-26/basic/transports#streamable-http) transport
...
Before deploying, you need a server that implements MCP Streamable HTTP transport. Make sure it has:
- `/mcp` endpoint to handle protocol messages
- CORS configuration
- Smithery config handling (optional)

Check out these examples and cookbooks to quickly get started:

- **[Migration Guides](/migrations)** - Convert existing STDIO servers to HTTP transport
- **[Cookbooks & Examples](/cookbooks)** - Complete working examples of servers
