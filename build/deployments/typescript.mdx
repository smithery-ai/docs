...
description: 'Deploy and publish TypeScript MCP servers on Smithery using Smithery CLI'
icon: 'braces'
---
...
<Note>
**Local servers are in beta** - When you set `target: "local"`, your server runs locally on your machine but is accessible through Smithery's registry for easy discovery and connection by MCP clients.
</Note>

For remote deployment, this minimal configuration is intentional - Smithery handles containerization and deployment automatically for TypeScript projects.
...
1. Your server runs locally on your machine using `npm run dev`
