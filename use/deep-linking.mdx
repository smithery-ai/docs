...
```typescript
`${clientScheme}://mcp/install?name=${encodeURIComponent(displayName)}&config=${encodeURIComponent(config)}`

// Example:
// <client-scheme>://mcp/install?<server-display-name>&<base64-encoded-json-config>
```

| **Component**       | **Description**                                       |
| :------------------ | :---------------------------------------------------- |
| `{clientSchema}://` | Your client protocol scheme                           |
| `/mcp/install`      | Path                                                  |
| `name`              | Query parameter for the server's display name         |
