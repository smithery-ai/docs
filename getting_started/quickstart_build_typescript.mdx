...
icon: "braces"
description: "Deploy your first Typescript Streamable HTTP MCP server in minutes"
---
...
In this quickstart, we'll build a simple MCP server that says _hello_ to users. We'll use the official [TypeScript MCP SDK](https://github.com/modelcontextprotocol/typescript-sdk) with the [Smithery CLI](https://github.com/smithery-ai/cli). By the end, you'll have a live, deployed server with built-in authentication that you can connect to from any MCP client. 
...
When you run the command, you'll be prompted with a few questions:

1. **Project name** - Enter a name for your MCP server project
2. **Local filesystem access** - Choose whether your server needs access to the local filesystem
   - Select **No** for this quickstart (this will run as a hosted server)
   - Select **Yes** if you're building a local-only server that runs on users' devices
3. **Template selection** - Choose your server template:
   - **basic** - A simple MCP server scaffold (recommended for this quickstart)
   - **chatgpt-app** - An OpenAI app scaffold (see the [ChatGPT App quickstart](/build/chatgpt-apps/quickstart) for details)

For this quickstart, select **No** for filesystem access and **basic** for the template.
