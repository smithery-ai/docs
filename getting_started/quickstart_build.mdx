...
description: "Build and run a Streamable HTTP MCP server in TypeScript with two lines of code"
---

The easiest way to build an MCP server is to use the TypeScript MCP SDK with the Smithery CLI. For other languages like Python, you can follow our [Python custom container guide](/cookbooks/python_custom_container) to build with FastMCP and Docker.

### Choose your hosting model

- Hosted on Smithery (recommended): Build and deploy on Smithery with CI/CD and a great developer experience.
- Self-hosted: Already running your MCP elsewhere? Publish it by URL. See [External MCPs](/build/external).
...
Or, you can invoke this server by prompting something like "Say hello to Henry".
...
This will port-forward your local server to the Smithery Playground via ngrok. You can also reference the [Smithery CLI](https://github.com/smithery-ai/cli) for more controls.
