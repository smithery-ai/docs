...
def count_characters(text: str, character: str) -> str:
    ...
    # Tool logic with server token validation, etc.
    pass
...
def count_characters(text: str, character: str) -> str:
    ...
    # Example: Get config values that users can pass to your server session
    server_token = get_config_value("serverToken")
    user_name = get_config_value("userName", "User") 
    ...
    return f'Hello {user_name}! The character "{character}" appears {count} times in the text.'
```
        ...
        if scope.get('type') == 'http':
            ...
            if 'config=' in query:
                try:
                    ...
                    # Store config in request scope for per-request access
...
<Note>
**Local vs Deployed Ports**: For local testing, you can use any port (like 8080 above). However, when deployed to Smithery, your server **must** listen on the `PORT` environment variable, which Smithery will set to 8080.
</Note>
