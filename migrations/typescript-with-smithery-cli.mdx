...
- Uses optional debug configuration for development
- Uses Smithery CLI for automatic HTTP transport and containerization

<Note>
**Full Implementation**: For the complete working character counter example with all dependencies and configuration files, see the [smithery-cookbook repository](https://github.com/smithery-ai/smithery-cookbook/blob/main/servers/typescript/typescript_quickstart/src/index.ts).
</Note>
    ...
    async ({ text, character }) => {
      ...
      return {
        content: [
          { 
            type: "text", 
            text: `The character "${character}" appears ${count} times in the text.${debugInfo}` 
          }
        ],
      };
    }
