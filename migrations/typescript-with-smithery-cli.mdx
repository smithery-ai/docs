...
Migration involves creating a file that exports an MCP server object using the official McpServer from "@modelcontextprotocol/sdk/server/mcp.js". This main server file (can be `src/index.ts`, `src/server.ts`, or any filename) must export your MCP server so the Smithery CLI can build and deploy it to run over HTTP. You specify which file to use by setting the `module` field in your `package.json`.
