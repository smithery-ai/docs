...
**What we're building**: A simple stateless MCP server with a `greet` tool that takes a user's name and returns a personalized greeting. The server validates an API key from Smithery configuration to demonstrate secure configuration handling.
