---
title: "Host a Python MCP Server with FastMCP and Docker"
...
## Step 3: Add Configuration Support (Optional)

Now let's add support for Smithery session configuration. This step has two parts:

### Step 3.1: Create Configuration Middleware

First, create the middleware that will extract config from URL parameters:
...
This middleware:
- Extracts configuration from JSON dot-notation query parameters
...
### Step 3.2: Add Configuration Handling

Now add configuration support to your main server file by importing the middleware and setting up config handling:
...
### Step 3.3: Enhance Tool with Configuration

Now update your tool to use the configuration validation:
...
<Note>
**Configuration is Optional**: If your MCP server doesn't need any configuration (API keys, settings, etc.), you can skip the config-related steps. The server will work perfectly fine without it.
</Note>

## Step 4: Complete Server Startup

Set up the HTTP server with CORS and middleware:

```python
# main.py
def main():
    # HTTP mode with config extraction from URL parameters
    print("Say Hello MCP Server starting in HTTP mode...")
    ...
    # Apply custom middleware for config extraction (optional)
